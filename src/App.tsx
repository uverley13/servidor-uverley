...
function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { setServices(initialServices); setLoading(false); }, 450);
  ...
  const login = (email: string, password: string) => {
    const match = [...defaultUsers, ...readStoredUsers()].find((item) => item.email.toLowerCase() === email.trim().toLowerCase() && item.password === password);
    if (!match) { setError('Correo o contraseña incorrectos.'); return; }
    setError(null); setUser(match); setShowLogin(false); navigate(match.role === 'admin' ? '/admin' : '/perfil');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      {home ? <><Hero /><ServicesSection services={services} loading={loading} error={error} /><BenefitsSection /><PricingSection /><ProcessSection /><FaqSection /><ContactSection /></> : null}
      ...
      {!home && route !== '/perfil' && route !== '/admin' ? <RouteNotFound /> : null}
      <Footer />
