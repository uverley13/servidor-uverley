    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/70 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 md:flex">
          {user ? (
            <>
              <button type="button" onClick={() => onNavigate('/perfil')} className="text-sm text-slate-200 transition hover:text-white">
                Perfil
              </button>
              <button
                type="button"
                onClick={onLogout}
                className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white transition hover:border-violet-400/60 hover:bg-violet-500/10"
              >
                Cerrar sesión
              </button>
            </>
            ...
            <div className="mt-3 flex items-center gap-3">
              {user ? (
                <button
                  ...
                  className="flex-1 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white"
                >
                  Cerrar sesión
                </button>
              ) : (
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-green-600 text-white"
              >
                <MessageCircleMore className="h-5 w-5" />
              </a>
            </div>
...
export function Hero() {
  return (
    <section id="inicio" className="section-shell relative overflow-hidden px-4 pb-16 pt-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <h1 className="mt-5 max-w-xl text-4xl font-black leading-tight tracking-[-0.05em] text-white sm:text-5xl md:text-6xl">
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a
                href="#servicios"
                className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 to-violet-500 px-6 py-3 font-semibold text-white shadow-xl shadow-violet-500/30 transition hover:scale-[1.02]"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-6 py-3 font-semibold text-white transition hover:border-violet-400/60 hover:bg-violet-500/10"
              >
...
export function ServicesSection({ services, loading, error }: { services: ServiceItem[]; loading: boolean; error: string | null }) {
  if (loading) {
    return (
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="soft-card rounded-[2rem] p-8 text-center text-slate-300">
  ...
  if (error) {
    return (
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="rounded-[1.75rem] border border-rose-500/30 bg-rose-500/10 p-6 text-sm text-rose-100">{error}</div>
        </div>
      </section>
    );
  }
  ...
  if (!services.length) {
    return (
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl rounded-[1.75rem] border border-dashed border-slate-700 bg-slate-900/50 p-8 text-center">
          <h3 className="text-lg font-semibold text-slate-100">No hay servicios disponibles</h3>
          <p className="mt-2 text-sm text-slate-400">Pronto activaremos más soluciones para ti.</p>
        </div>
      </section>
    );
  }
            ...
            return (
              <article key={service.id} className="group rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-5 transition duration-300 hover:-translate-y-1 hover:border-violet-400/50 hover:shadow-[0_24px_60px_rgba(109,40,217,0.22)]">
                <div className="flex items-center justify-between">
                  ...
                  {service.active ? (
                    <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[10px] text-emerald-300">Activo</span>
                  ) : (
                    <span className="rounded-full border border-slate-500/30 bg-slate-500/10 px-2 py-1 text-[10px] text-slate-300">Inactivo</span>
                  )}
                </div>
                ...
                <div className="mt-5 flex items-center justify-between border-t border-white/10 pt-4">
                  ...
                  <button type="button" className="inline-flex items-center gap-2 rounded-full border border-violet-500/40 bg-violet-500/10 px-3 py-2 text-sm font-medium text-violet-100 transition hover:bg-violet-500/20">
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`rounded-[2rem] border p-6 ${plan.popular ? 'border-violet-400/60 bg-gradient-to-b from-violet-500/15 to-slate-900/70 shadow-[0_20px_60px_rgba(109,40,217,0.25)]' : 'border-white/10 bg-slate-900/70'}`}
            >
              {plan.popular ? (
                <div className="mb-4 inline-flex rounded-full border border-violet-300/40 bg-violet-500/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-violet-200">
                  Más popular
                </div>
              ) : null}
              ...
              <button
                type="button"
                className={`mt-6 inline-flex w-full items-center justify-center rounded-full px-4 py-3 font-semibold text-white ${plan.popular ? 'bg-gradient-to-r from-violet-500 to-cyan-500' : 'border border-white/10 bg-white/5 hover:bg-violet-500/10'}`}
              >
                {plan.cta}
              </button>
            </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {steps.map((step, index) => (
            <div key={step} className="relative rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-5">
              <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-violet-500/20 to-cyan-500/20 text-lg font-bold text-violet-200">
                {index + 1}
              </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            ...
            <div className="mt-6 space-y-4 text-sm text-slate-200">
              <p>
                WhatsApp: <a href="https://wa.me/573172329884" className="font-semibold text-violet-200">+57 317 232 9884</a>
              </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 rounded-[1.75rem] border border-white/10 bg-slate-950/30 p-4 sm:p-5">
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-violet-400 focus:outline-none"
            ...
            <div>
              ...
              <textarea
                ...
                className="min-h-28 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder:text-slate-500 focus:border-violet-400 focus:outline-none"
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-6">
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-300">
            ...
            <li><span className="text-slate-400">Rol:</span> {user.role === 'admin' ? 'Administrador' : 'Cliente'}</li>
            <li><span className="text-slate-400">Saldo disponible:</span> ${user.balance.toLocaleString('es-CO')}</li>
          </ul>
          <button type="button" onClick={onLogout} className="mt-6 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium text-white">Cerrar sesión</button>
        </div>
        <div className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-6">
          ...
          <div className="mt-4 space-y-3">
            {user.orders.map((order) => (
              <div key={order.id} className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-200">
                <div className="flex justify-between gap-2"><span>{order.service}</span><span className="text-violet-200">{order.status}</span></div>
          ...
          <div className="mt-4 space-y-3">
            {['Servicios digitales', 'Gestión de pedidos', 'Atención por WhatsApp'].map((service) => (
              <div key={service} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-200">
                <span>{service}</span>
                <button type="button" className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-[10px] uppercase tracking-[0.12em] text-emerald-200">
                  Activo
                </button>
              </div>
            ))}
            ...
            {['REC-102', 'REC-204', 'REC-187'].map((item) => (
              <div key={item} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-slate-200">
                ...
                <div className="flex gap-2">
                  <button type="button" className="rounded-full bg-emerald-500/15 px-3 py-1 text-[10px] uppercase tracking-[0.12em] text-emerald-200">Aprobar</button>
                  <button type="button" className="rounded-full bg-rose-500/15 px-3 py-1 text-[10px] uppercase tracking-[0.12em] text-rose-200">Rechazar</button>
                </div>
              </div>
            ))}
          </div>
...
export function RouteNotFound({ message = 'La ruta que buscas no existe o fue movida.' }: { message?: string }) {
  return (
    <div className="mx-auto flex min-h-[60vh] max-w-3xl items-center justify-center px-4 py-20 text-center">
      <div className="rounded-[2rem] border border-white/10 bg-slate-900/70 p-8">
        ...
        <button type="button" onClick={() => window.history.pushState({}, '', '/')} className="mt-6 inline-flex rounded-full bg-gradient-to-r from-violet-500 to-cyan-500 px-5 py-3 font-semibold text-white">
          Volver al inicio
        </button>
      </div>
    </div>
